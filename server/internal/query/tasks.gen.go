...
func newTask(db *gorm.DB, opts ...gen.DOOption) task {
	...
	_task.ID = field.NewField(tableName, "id")
	...
	_task.OwnerID = field.NewField(tableName, "owner_id")
	_task.WorkflowID = field.NewField(tableName, "workflow_id")
	_task.Jobs = taskHasManyJobs{
	...
	_task.Owner = taskBelongsToOwner{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Owner", "models.User"),
	}

	_task.Workflow = taskBelongsToWorkflow{
		db: db.Session(&gorm.Session{}),
...
type task struct {
	...
	Jobs       taskHasManyJobs

	Owner taskBelongsToOwner

	Workflow taskBelongsToWorkflow
...
func (t *task) updateTableName(table string) *task {
	...
	t.WorkflowID = field.NewField(table, "workflow_id")
...
func (t task) clone(db *gorm.DB) task {
	...
	t.Jobs.db = db.Session(&gorm.Session{Initialized: true})
	t.Jobs.db.Statement.ConnPool = db.Statement.ConnPool
	t.Owner.db = db.Session(&gorm.Session{Initialized: true})
	t.Owner.db.Statement.ConnPool = db.Statement.ConnPool
	t.Workflow.db = db.Session(&gorm.Session{Initialized: true})
	t.Workflow.db.Statement.ConnPool = db.Statement.ConnPool
	return t
}

func (t task) replaceDB(db *gorm.DB) task {
	t.taskDo.ReplaceDB(db)
	t.Jobs.db = db.Session(&gorm.Session{})
	t.Owner.db = db.Session(&gorm.Session{})
	t.Workflow.db = db.Session(&gorm.Session{})
	return t
}

type taskHasManyJobs struct {
	db *gorm.DB
...
func (a taskHasManyJobs) Where(conds ...field.Expr) *taskHasManyJobs {
	if len(conds) == 0 {
		return &a
	}
...
func (a taskHasManyJobs) WithContext(ctx context.Context) *taskHasManyJobs {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a taskHasManyJobs) Session(session *gorm.Session) *taskHasManyJobs {
	a.db = a.db.Session(session)
	return &a
}

func (a taskHasManyJobs) Model(m *models.Task) *taskHasManyJobsTx {
	return &taskHasManyJobsTx{a.db.Model(m).Association(a.Name())}
}

func (a taskHasManyJobs) Unscoped() *taskHasManyJobs {
	a.db = a.db.Unscoped()
	return &a
}

type taskHasManyJobsTx struct{ tx *gorm.Association }

func (a taskHasManyJobsTx) Find() (result []*models.Job, err error) {
	return result, a.tx.Find(&result)
}

func (a taskHasManyJobsTx) Append(values ...*models.Job) (err error) {
...
func (a taskHasManyJobsTx) Replace(values ...*models.Job) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskHasManyJobsTx) Delete(values ...*models.Job) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskHasManyJobsTx) Clear() error {
	return a.tx.Clear()
}

func (a taskHasManyJobsTx) Count() int64 {
	return a.tx.Count()
}

func (a taskHasManyJobsTx) Unscoped() *taskHasManyJobsTx {
	a.tx = a.tx.Unscoped()
	return &a
}

type taskBelongsToOwner struct {
	db *gorm.DB

	field.RelationField
}

func (a taskBelongsToOwner) Where(conds ...field.Expr) *taskBelongsToOwner {
	if len(conds) == 0 {
		return &a
	}
...
func (a taskBelongsToOwner) WithContext(ctx context.Context) *taskBelongsToOwner {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a taskBelongsToOwner) Session(session *gorm.Session) *taskBelongsToOwner {
	a.db = a.db.Session(session)
	return &a
}

func (a taskBelongsToOwner) Model(m *models.Task) *taskBelongsToOwnerTx {
	return &taskBelongsToOwnerTx{a.db.Model(m).Association(a.Name())}
}

func (a taskBelongsToOwner) Unscoped() *taskBelongsToOwner {
	a.db = a.db.Unscoped()
	return &a
}

type taskBelongsToOwnerTx struct{ tx *gorm.Association }

func (a taskBelongsToOwnerTx) Find() (result *models.User, err error) {
	return result, a.tx.Find(&result)
}

func (a taskBelongsToOwnerTx) Append(values ...*models.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToOwnerTx) Replace(values ...*models.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToOwnerTx) Delete(values ...*models.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToOwnerTx) Clear() error {
	return a.tx.Clear()
}

func (a taskBelongsToOwnerTx) Count() int64 {
	return a.tx.Count()
}

func (a taskBelongsToOwnerTx) Unscoped() *taskBelongsToOwnerTx {
	a.tx = a.tx.Unscoped()
	return &a
}

type taskBelongsToWorkflow struct {
	db *gorm.DB

	field.RelationField
}

func (a taskBelongsToWorkflow) Where(conds ...field.Expr) *taskBelongsToWorkflow {
	if len(conds) == 0 {
		return &a
	}
...
func (a taskBelongsToWorkflow) WithContext(ctx context.Context) *taskBelongsToWorkflow {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a taskBelongsToWorkflow) Session(session *gorm.Session) *taskBelongsToWorkflow {
	a.db = a.db.Session(session)
	return &a
}

func (a taskBelongsToWorkflow) Model(m *models.Task) *taskBelongsToWorkflowTx {
	return &taskBelongsToWorkflowTx{a.db.Model(m).Association(a.Name())}
}

func (a taskBelongsToWorkflow) Unscoped() *taskBelongsToWorkflow {
	a.db = a.db.Unscoped()
	return &a
}

type taskBelongsToWorkflowTx struct{ tx *gorm.Association }

func (a taskBelongsToWorkflowTx) Find() (result *models.Workflow, err error) {
	return result, a.tx.Find(&result)
}

func (a taskBelongsToWorkflowTx) Append(values ...*models.Workflow) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToWorkflowTx) Replace(values ...*models.Workflow) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToWorkflowTx) Delete(values ...*models.Workflow) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToWorkflowTx) Clear() error {
	return a.tx.Clear()
}

func (a taskBelongsToWorkflowTx) Count() int64 {
	return a.tx.Count()
}

func (a taskBelongsToWorkflowTx) Unscoped() *taskBelongsToWorkflowTx {
	a.tx = a.tx.Unscoped()
	return &a
}
