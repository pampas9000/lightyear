...
import { LockKeyhole, AlertCircle, Loader2 } from 'lucide-vue-next'
...
<template>
    <Dialog :open="showAuthModal" @update:open="close">
        <DialogContent class="sm:max-w-[400px] p-0 overflow-hidden border-none shadow-2xl rounded-[24px] bg-white dark:bg-slate-900">
            <div class="relative w-full p-8">
                <div class="mb-8 text-center">
                    ...
                    <DialogHeader>
                        <DialogTitle class="text-2xl font-black tracking-tight text-slate-900 dark:text-white text-center uppercase">
                            {{ mode === 'login' ? 'Welcome back' : 'Create Account' }}
                        </DialogTitle>
                        <DialogDescription class="mt-2 text-xs font-medium text-slate-400 dark:text-slate-500 text-center max-w-[240px] mx-auto leading-relaxed">
                            {{ mode === 'login'
                ...
                <form @submit.prevent="handleSubmit" class="space-y-4">
                    ...
                    <div class="space-y-1.5">
                        <Label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Username</Label>
                    ...
                    <div v-if="mode === 'register'" class="space-y-1.5">
                        <Label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Email</Label>
                    ...
                    <div class="space-y-1.5">
                        <Label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Password</Label>
                        <Input v-model="password" type="password" required
                            class="h-11 rounded-xl bg-slate-50 dark:bg-slate-800/50 border-transparent focus:border-blue-500/50 focus:bg-white dark:focus:bg-slate-800 px-4 text-sm font-bold transition-all duration-300"
                            placeholder="••••••••" />
                    </div>

                    <Button type="submit" :disabled="loading" class="w-full h-12 rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-blue-600/10 mt-2 bg-blue-600 hover:bg-blue-700 hover:translate-y-[-1px] active:translate-y-[1px] transition-all duration-300">
                ...
                <div class="mt-6 flex items-center gap-4">
                    ...
                    <span class="text-[8px] text-slate-300 dark:text-slate-600 uppercase tracking-widest font-black">Social Connect</span>
                    <Separator class="flex-1 opacity-50" />
                </div>

                <div class="mt-6 grid grid-cols-2 gap-3">
                    <Button @click="handleOAuth('google')" type="button" variant="outline" class="h-11 rounded-xl gap-2 font-bold text-[10px] uppercase tracking-widest border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-300">
                    ...
                    <Button @click="handleOAuth('github')" type="button" variant="outline" class="h-11 rounded-xl gap-2 font-bold text-[10px] uppercase tracking-widest border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-300">
                ...
                <div class="mt-8 text-center">
                    <button @click="toggleMode"
                        ...
                        <span class="text-blue-600 dark:text-blue-400 ml-1 group-hover:ml-1.5 transition-all underline decoration-2 underline-offset-4 decoration-blue-500/20">
