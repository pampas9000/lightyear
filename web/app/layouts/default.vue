            ...
            <nav class="flex-1 space-y-1">
                <NuxtLink v-for="item in navItems" :key="item.to" :to="item.to">
                    <Button variant="ghost"
                        class="w-full justify-start gap-2.5 px-3.5 h-9 text-sm font-medium rounded-lg transition-all cursor-pointer"
                        :class="[
                            $route.path === item.to
            ...
            <div class="mt-auto border-t border-hairline pt-4 space-y-1">
                ...
                <div v-else class="mt-2">
                    <Button @click="showAuthModal = true" variant="outline"
                        class="w-full h-9 font-medium text-sm shadow-sm hover:bg-surface-2 rounded-lg border-hairline cursor-pointer">
                        {{ $t('common.login') }}
                    </Button>
                </div>
            </div>
