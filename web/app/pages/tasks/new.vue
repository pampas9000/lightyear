...
onMounted(() => {
  ...
  uppy.on('upload-progress', (file, progress) => {
    ...
    if (f) {
      f.progress = Math.round((progress.bytesUploaded / progress.bytesTotal) * 100)
    }
  })
  ...
  uppy.on('upload-success', (file, response) => {
    ...
    const key = file.s3Multipart?.key || response.body?.key || (response.body as any)?.location?.split('/').slice(2).join('/')
...
<template>
  <div class="max-w-[1000px] mx-auto w-full">
    ...
    <div class="mb-10">
      ...
      <p class="text-sm text-slate-500 dark:text-slate-400">Upload media files and configure bulk transcoding parameters.</p>
    </div>

    <div class="space-y-8">
      <!-- Parameters Section -->
      <section class="bg-white dark:bg-slate-900 rounded-xl p-8 shadow-sm border border-slate-100 dark:border-slate-800">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-white mb-6">Transcoding Parameters</h2>
        
        <div class="grid gap-8 md:grid-cols-2">
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Target Format</label>
            <div class="relative">
              <select v-model="targetFormat" class="w-full appearance-none rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-2.5 text-sm font-medium text-slate-900 dark:text-white focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Quality (0-100)</label>
            <input type="number" v-model="quality" min="0" max="100" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-2.5 text-sm font-medium text-slate-900 dark:text-white focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors" />
          </div>
        </div>
      </section>
      ...
      <section class="bg-white dark:bg-slate-900 rounded-xl p-8 shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        <div v-if="files.length > 0" class="space-y-3">
          <div v-for="file in files" :key="file.id" class="flex items-center justify-between p-4 rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 group transition-colors hover:border-slate-200 dark:hover:border-slate-700">
            <div class="flex items-center space-x-4 truncate flex-1 pr-4">
              <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 shadow-sm">
              ...
              <div class="flex-1 truncate">
                ...
                <p class="text-xs text-slate-500 mt-0.5">{{ (file.size / 1024 / 1024).toFixed(2) }} MB</p>
              </div>
            </div>
            ...
            <div class="flex items-center space-x-4">
              <div v-if="isUploading || file.status === 'success'" class="w-24">
                <div class="h-1.5 w-full rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                  <div class="h-full bg-blue-600 transition-all duration-300" :style="{ width: `${file.progress}%` }" />
                </div>
              </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-16 px-4 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50">
          <div class="h-12 w-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-4 text-slate-500">
             <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>
          </div>
        ...
        <div v-if="files.length > 0" class="pt-8 mt-8 border-t border-slate-100 dark:border-slate-800">
          <div v-if="isUploading" class="space-y-2 mb-6">
            ...
            <div class="h-2 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden shadow-inner">
              <div class="h-full bg-blue-600 transition-all duration-300" :style="{ width: `${globalProgress}%` }" />
            </div>
          </div>
          ...
          <div class="flex justify-end">
            ...
            >
              ...
              {{ isUploading ? 'Processing Upload...' : (uploadComplete ? 'Task Created' : 'Start Processing') }}
            </button>
          </div>
        </div>
      </section>
